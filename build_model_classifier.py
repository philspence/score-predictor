...
def build_model():
    ...
    model.add(Dense(128, activation='relu', input_dim=2))  # Dense layer of 16 nodes, activation used is relu
    ...
    model.add(Dense(2, activation='softmax'))  # output has a shape of (-1, 2), -1 is just the length of the array i.e.
    ...
    model.compile(loss='categorical_crossentropy', optimizer='adam',  metrics=['categorical_accuracy'])  # compile the
    # model, using caterogical_crossentropy as the loss (monitors how accurate the model is, lower is better),
    # adam as the optimiser and then outputs caterogical_accuracy for you to see as it trains (lower is better)
    return model


def load_data(infile):
    data = pd.read_csv(f'{infile}.csv', header=0)  # load the CSV of the data as a dataframe, uses the first row as the
    # headers for each column
    X = data[['HPAvg', 'APAvg']].to_numpy()  # use the headers to extract the correct columns...
    ...
    y = data[['HW', 'AW']].to_numpy()  # this is the 2D matrix, a home win = [1, 0, 0], away win = [0, 1, 0] the model
...
def main(inf, m_name):
    ...
    model.fit(X_train, y_train, epochs=10, verbose=1, validation_data=(X_test, y_test))  # fit the model to the
    # training dataset X and y, validate it with testing X and y. Epochs is how many times it iterates over itself
    # this can be trained
    y_pred = model.predict(X)  # put all of X backthrough it and get the predictions
    data['HWPred'], data['AWPred']= y_pred[:, 0], y_pred[:, 1]  # split the array of y_pred into H and A, i.e. takes
    # [[1, 0], [0, 1], [1, 0]] and takes the first value of each prediction (1, 0 and 1, in this case) and makes that
    ...
    print(data.head(10))  # prints the first 10 and then the last 10 rows of the dataframe
    print(data.tail(10))
    model.save(m_name)  # saves the model using the -m value given when running the script
