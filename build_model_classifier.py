...
def build_model():
    early_stopping = [EarlyStopping(monitor='val_loss', patience=5),
                      ModelCheckpoint(filepath='cat_best_model', monitor='val_loss',
                                      save_best_only=True)]
    model = Sequential()  # sequential model
    model.add(Dense(24, activation='relu', input_dim=4))  # Dense layer of 16 nodes, activation used is relu
...
def load_data(infile):
    ...
    X = data[['HPAvg', 'APAvg', 'HPMean', 'APMean']].to_numpy()  # use the headers to extract the correct columns...
...
def main(inf, m_name):
    ...
    print(data.head(20))  # prints the first 20 and then the last 20 rows of the dataframe
    print(data.tail(20))
